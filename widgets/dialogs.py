...
class DlgSetting(QDialog):
    def __init__(self, res: AppRes, code: str, dict_setting: dict):
        ...
        self.setStyleSheet("QDialog {font-family: %s;}" % res.font_mono.name)
        ...
        lab_head_name.setStyleSheet("QLabel {font-family: %s;}" % res.font_mono.name)
        layout.addWidget(lab_head_name, r, 0)

        lab_head_value = LabelRaised("設定値")
        lab_head_value.setStyleSheet("QLabel {font-family: %s;}" % res.font_mono.name)
        layout.addWidget(lab_head_value, r, 1)

        for key, value in self.dict_setting.items():
            r += 1
            lab_param = LabelRaisedLeft(key)
            lab_param.setStyleSheet("QLabel {font-family: %s;}" % res.font_mono.name)
            layout.addWidget(lab_param, r, 0)
            ent_param = EntrySetting(str(value))
            ent_param.setStyleSheet("QLineEdit {font-family: %s;}" % res.font_mono.name)
