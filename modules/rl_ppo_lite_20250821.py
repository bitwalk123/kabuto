...
class TradingSimulation:
    ...
    def __init__(self,
        ...
        if loaded:
            ...
            self.logger.info(f"{__name__}: [model] 既存モデルを読み込みました → 使用")
        else:
            # print("[model] 既存モデルなし/不一致 → 新規作成して保存予定")
            self.logger.info(f"{__name__}: [model] 既存モデルなし/不一致 → 新規作成して保存予定")
    ...
    def add(self, ts: float, price: float, volume: float, force_close: bool = False) -> str:
        ...
        if len(self.agent.obs_buf) >= self.agent.cfg.rollout_length or done:
            try:
                ...
                self.logger.warning(f"{__name__}: [warn] PPO更新に失敗:")
