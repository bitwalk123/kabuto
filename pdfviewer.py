...
class PDFViewer(QMainWindow):
    def __init__(self):
        ...
        self.action_save = action_save = QAction(icon_save, "Save Current Page", self)
        action_save.triggered.connect(self.on_save_page)
    ...
    def on_save_page(self):
        if self.doc is None or not self.path_pdf:
            return
        ...
        # 元ファイル名
        base = os.path.splitext(os.path.basename(self.path_pdf))[0]

        digits = len(str(total))
        default_name = f"{base}_{page + 1:0{digits}d}.pdf"
        ...
        try:
            ...
            QMessageBox.information(self, "Saved", f"Saved: {path_save}")

        except Exception as e:
            QMessageBox.warning(self, "Error", f"Failed to save page:\n{e}")
